...
namespace myakish::tree
{
    ...
    struct ChildrenBuilder
    {
        ...
        struct promise_type
        {
            ...
            template<std::convertible_to<Handle> ReceivedHandle>
            std::suspend_always yield_value(ReceivedHandle&& handle)
            {
                currentHandle = std::forward<ReceivedHandle>(handle);
                return {};
            }
            void return_void() {}
        ...
        const Handle& CurrentHandle() const
        {
    ...
    struct CommonHandle : meta::Undefined {};

    template<HasHandleType First, typename Second>
    struct CommonHandle<First, Second> : HandleType<First> {};

    template<typename First, HasHandleType Second>
    struct CommonHandle<First, Second> : HandleType<Second> {};

    template<HasHandleType First, HasHandleType Second>
    struct CommonHandle<First, Second> :
        std::conditional<
            std::same_as<
                typename HandleType<First>::type,
                typename HandleType<Second>::type>,
            typename HandleType<First>::type,
            meta::UndefinedType> {};
    ...
    {
        { source.BuildChildren(storage) } -> std::same_as<ChildrenBuilder<typename HandleType<Type>::type>>;
    };
    ...
    struct WithChild : CopyHandleType<Target>
    {
        ...
        template<HandleConcept Handle>
        ChildrenBuilder<typename HandleType<Child>::type> BuildChildren(Storage<Handle>& storage) const requires !HasChildren<Target>
        {
        ...
        template<HandleConcept Handle>
        ChildrenBuilder<typename CommonHandle<Target, Child>::type> BuildChildren(Storage<Handle>& storage) const requires HasChildren<Target>
        {
